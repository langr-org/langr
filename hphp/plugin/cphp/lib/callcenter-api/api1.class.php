 ...
 * 
 * $Id: api1.class.php 536 2012-04-20 01:42:42Z huangh $
 */
...
class api extends ami
{
	 ...
	 * @modify by Langr <hua@langr.org> 2012/01/31 16:38
	 * 	asterisk 接口改变, 不再需要应用层保存通道信息.
	 * 	只需要传递工号, 不需要处理分机号.
	 * @param $args['method']
	 * @param $args['return']
	...
	function showCallKeep(&$args = array()) /* {{{ */
	{
		...
		if ( $args['type'] != 1 && $_SESSION['userinfo'][$exten]['keep'] != 1 ) {
			$rs = lagi_action("Action: keepcall\r\nSrcExten: $exten\r\n\r\n");

			if ( $rs['Response'] != 'Success' ) {
				$ret = self::_error(self::E_OP_FAIL);
			} else {
		...
		/* 恢复 */
			...
			$rs = lagi_action("Action: restorepark\r\nSrcExten: $exten\r\n\r\n");
			$ret = self::_error(self::E_OK);
			//$ret['ret'] = $rs;
			$ret['ret']['keep'] = 0;
	...
	function old_showCallKeep(&$args = array()) /* {{{ */
	{
		$_this = & self::_getInstance();
		if ( $_this == false ) {
			return self::_return(self::_error(self::E_LOGOFF), $args['method'], $args['return']);
	...
	function showCallFeedBack(&$args = array()) /* {{{ */
	{
		...
		$exten = $args['srcno'];
		$rs = lagi_action("Action: feedback\r\nSrcExten: $exten\r\n\r\n");
		if ( $rs['Response'] != 'Success' ) {
			$ret = self::_error(self::E_OP_FAIL);
		} else {
			$ret = self::_error(self::E_OK);
	...
	function old_showCallFeedBack(&$args = array()) /* {{{ */
	{
		$_this = & self::_getInstance();
		if ( $_this == false ) {
			return self::_return(self::_error(self::E_LOGOFF), $args['method'], $args['return']);
	...
	function showCallTransfer(&$args = array()) /* {{{ */
	{
		...
		$rs = lagi_action("Action: atredirect\r\nSrcExten: ".$args['srcno']."\r\nExten: ".$args['dstno']."\r\n\r\n");
		$ret = self::_error(self::E_OK);
		$ret['ret'] = $rs;
		return $_this->_return($ret, $args['method'], $args['return']);
	} /* }}} */

	function old_showCallTransfer(&$args = array()) /* {{{ */
	{
		$_this = & self::_getInstance();
		if ( $_this == false ) {
			return self::_return(self::_error(self::E_LOGOFF), $args['method'], $args['return']);
	...
	function showMultiCall(&$args = array()) /* {{{ */
	{
		...
		$rs = lagi_action("Action: threewaycalling\r\nSrcExten: ".$args['srcno']."\r\nExten: ".$args['dstno']."\r\n\r\n");
		$ret = self::_error(self::E_OK);
		$ret['ret'] = $rs;
		return $_this->_return($ret, $args['method'], $args['return']);
	} /* }}} */

	function old_showMultiCall(&$args = array()) /* {{{ */
	{
		$_this = & self::_getInstance();
		if ( $_this == false ) {
			return self::_return(self::_error(self::E_LOGOFF), $args['method'], $args['return']);
	...
	function showPopWindow(&$args = array()) /* {{{ */
	{
		...
		if ( API_DEBUG == 'debug' ) {
			$_this->wlog('pop1.log', $_SERVER["REQUEST_URI"]);
		}
	...
	function &_getInstance($action = null) /* {{{ */
	{
		...
		if ( $action != 'PopWindow' && !$instance->linkid ) {
			$instance->linkid = lagi_connect(CFG_AGI_HOST_PBX, '5038', CFG_AGI_USER_PBX, CFG_AGI_PWD_PBX);
		}
		return $instance;
	} /* }}} */
