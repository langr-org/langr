 ...
 * 登陆信息:
 * 	Cookie['callbar_extension'] 登陆者的分机号(或工号).
 * 	Cookie['callbar_name'] 登陆者的名称.
 * 	!Cookie['callbar_login_exten'] 被登陆的分机, 用于在动态登陆时.
 * 	!$_COOKIE['callbar_remember_extension'] 记住分机为登陆状态, 下次免登陆
 * 	X!Cookie['callbar_key'] 当前用户的校验key: md5(CLIENT_IP+'@'+api_key);
 * 导航条自己设置的:
 * 	Cookie['agent'] 示忙标志, 1 已经示忙, 0 未示忙.
 * 	Cookie['keep'] 通话保持标志, 1 已经保持, 0 未保持.
 *
 * @author Langr <hua@langr.org> 2012/01/12 12:05
 * $Id: callcenter.php 537 2012-04-20 01:45:25Z huangh $
 */
/*
session_start();
 */
/* 不同应用配置这里: */
include(dirname(__FILE__).'/bar.config.php');
$pop_url = $config['p_url'];
$target_frame = $config['p_target'];
$type = $config['p_type'];
//$target_frame = '_blank';
...
if ( empty($_COOKIE['callbar_key']) ) {
	setcookie('callbar_key', $key, 0, '/');
}
if ( !empty($_COOKIE['callbar_remember_extension']) ) {
	setcookie('callbar_extension', $_COOKIE['callbar_remember_extension'], 0, '/');
}
...
function popWin()
{
	...
	//var chksum = '&chksum=' + Crypto.MD5(action+srcno+key);
	var chksum = '&chksum=' + '<?php echo md5('PopWindow'.$_COOKIE['callbar_extension'].''.$key);?>';
	var param = "action=PopWindow&srcno=<?php echo $_COOKIE['callbar_extension'];?>&type=3" + chksum;
	var myAjax = $.ajax({
			type: 'GET',
			url: url,
			data: param,
...
/* 第一次登陆时, 取分机状态(SetCookie('callbar_agent')), 检测并处理动态登陆分机. */
if ( !isset($_COOKIE['callbar_agent']) ) {
	$api_param = "?action=GetDB&type=DND&srcno=".$_COOKIE['callbar_extension'];
	$ret = json_decode(file_get_contents($api_url.$api_param));
	$ret = (array) $ret;
	setcookie('callbar_agent', $ret['ret'], 0, '/');
	//echo "<!--('".print_r($ret,true)."')-->";

	/* 动态登陆, 设置 asterisk DB */
	/*$api_param = "?action=GetDB&type=&srcno=".$_COOKIE['callbar_extension'];
	$ret = json_decode(file_get_contents($api_url.$api_param));
	$ret = (array) $ret;*/
	if ( !empty($_COOKIE['callbar_login_exten']) && strlen($_COOKIE['callbar_login_exten']) > 2 ) {
		if ( $ret['errno'] == 0 ) {
...
var key_ck = GetCookie('callbar_key');
var key = key_ck ? key_ck : "<?php echo $key; ?>";
var srcno = GetCookie('callbar_extension');
...
SetCookie('callbar_key', key, null, '/');
SetCookie('callbar_extension', srcno, null, '/');
*/
...
function extension_name()
{
	if ( srcno == null || srcno.length < 2 ) {
		...
		var dynamic_exten = GetCookie('callbar_login_exten') ? '('+GetCookie('callbar_login_exten')+')' : '';
		$('#extension_name').html("姓名:"+GetCookie('callbar_name')+"&nbsp;分机:"+srcno+dynamic_exten);
		$('#user_status').html(GetCookie('callbar_agent') ? '示忙' : '在线');
	}
}
...
#top_div {
	/*background:url(images/top/top_bg_1.gif) repeat-x;*/
	width: 100%;
	height: 34px;
	float: right;
}
...
#input_box,#div_help_msg,#user_info {
	padding-top: 6px;
	height: 34px;
}
#user_info {
...
<body style="background-color:transparent;border:none;margin:0px;">
<div id="top_div">
	<div class="td_div" id="callcenter_logo">
	<a href='<?php echo $index_page;?>' target='<?php echo $target_frame;?>'><img src="images/top/top_s_02.gif" height="30" alt="" border="0" ></a>
	</div>

	<div class="space_div">&nbsp;</div>
	<div class="td_div"><a href='<?php echo $index_page;?>' target='<?php echo $target_frame;?>'><img src="images/top/gohome.gif" alt="" class="nav_img"/></a></div>
	<div class="space_div">&nbsp;</div>
	<div class="td_div"><a href="javascript:setAgent();" style="text-decoration: none"><img src="images/top/btn_busy.gif" id="imgbusy" name="imgbusy" class="nav_img"/></a></div>
	<div class="space_div">&nbsp;</div>
	<div class="td_div"><a target="_self" href="javascript:setKeep();" ><img src="images/top/btn_bc.gif" name="imgkeep" id="imgkeep" class="nav_img"></a></div>
	<div class="space_div">&nbsp;</div>
	<div class="td_div"><a onMouseOver="MM_swapImage('imgzj','','images/top/btn_zj.gif',1)" onMouseOut="MM_swapImage('imgzj','','images/top/btn_zj.gif',1)"  target="_self" href="javascript:feedback();" ><img src="images/top/btn_zj.gif" name="imgzj" id="imgzj" class="nav_img"/></a></div>
	<div class="space_div">&nbsp;</div>
	<div class="td_div">
	    <a id="trans_id" href="javascript:;" title="转移"><img src="images/top/btn_transfer.gif" class="nav_img" alt="转移"/></a>
	</div>

	<div class="space_div">&nbsp;</div>
	<div class="td_div">
	    <a id="three_id" href="javascript:;" title="三方通话"><img src="images/top/help04.gif" class="nav_img" alt="三方通话"/></a>
	</div>
	<div class="space_div">&nbsp;</div>
	<div id="input_box" class="td_div td_div_text">
		<div style="float:left;font-size:12pt"><span id="dstinfo" wtalk="OutCall" lock="off">拔:</span><input id="dstno" name="dstno" class="input" type="text" size="11" height="34" border="0"/></div><div name="cc" id="cc" style="float:left;margin-left:2px;cursor: pointer;border: 1px solid #80bdcb;" onClick="clickCall();return false;">&nbsp;<img src="images/top/dial.gif" border=0 alt="快速呼叫">&nbsp;</div>
	</div>
		<!--div width="50">
	...
	<div id="div_help_msg">
	   <div id="help_msg" class="td_div"></div>
	</div>
	<div id="user_info" class="td_div">
		<div id="user_stauts" style="color:red;float:right;padding: 0px 2px;">在线</div>
		<div id="extension_name" style="float:right;padding: 0px 2px;">姓名：&nbsp;分机：&nbsp;</div>
	</div>
	<!--<div class="space_div">&nbsp;</div>
	    <div class="td_div">
...
function clickCall()
{
	...
	var url = api_url;
	var chksum = '&chksum=' + Crypto.MD5(action+srcno+dstno+key);
	var param = "action="+action+"&srcno="+srcno+"&dstno="+dstno+"&type=" + GetCookie('callbar_agent') + chksum;
	var myAjax = $.ajax({
			type: 'GET',
			url: url,
			data: param,
...
{
	var url = api_url;
	var action = 'AgentBusy';
	var chksum = '&chksum=' + Crypto.MD5(action+srcno+key);
	var param = "action="+action+"&srcno="+srcno+"&type=" + GetCookie('callbar_agent') + chksum;
	var myAjax = $.ajax({
			type: 'GET',
			url: url,
			data: param,
...
function setKeep()
{
	...
	var action = 'CallKeep';
	var chksum = '&chksum=' + Crypto.MD5(action+srcno+key);
	var param = "action="+action+"&srcno="+srcno+"&dstno=&type=" + GetCookie('callbar_keep') + chksum;
...
function feedback() {
	var url = api_url;
	var action = 'CallFeedback';
	var chksum = '&chksum=' + Crypto.MD5(action+srcno+key);
	var param = "action="+action+"&srcno="+srcno+"&dstno=" + chksum;
	var myAjax = $.ajax({
			type: 'GET',
			url: url,
			data: param,
...
function extenExit()
{
	...
	var action = 'Logout';
	var chksum = '&chksum=' + Crypto.MD5(action+srcno+key);
	var param = "action="+action+"&srcno="+srcno + chksum;
	var myAjax = $.ajax({
			type: 'GET',
			url: url,
			data: param,
...
function _help_msg(msg, flag)
{
	...
	$("#help_msg").html(msg);
	$("#help_msg").animate({"height": "18px","top": "0px"}, 1000).animate({"height": "18px","top": "0px"}, 3000).animate({"opacity": "0.1","height": "0px","top": "18px"},1000, _help_msg_donoe);
}
...
{
	if (GetCookie('callbar_agent') != 1) {
		SetCookie('callbar_agent', 1, null, '/');
		$('#imgbusy').attr({src: "images/top/btn_idle.gif"});
		$('#user_stauts').html('示忙');
		_help_msg("示忙返回,"+data.errmsg, data.errno);
	} else {
		SetCookie('callbar_agent', 0, null, '/');
		$('#imgbusy').attr({src: "images/top/btn_busy.gif"});
		$('#user_stauts').html('在线');
...
function keepDone(data)
{
	...
	if (GetCookie('callbar_keep') != 1) {
		SetCookie('callbar_keep', 1, null, '/');
		$('#imgkeep').attr({src: "images/top/btn_hf.gif"});
	} else {
		SetCookie('callbar_keep', 0, null, '/');
		$('#imgkeep').attr({src: "images/top/btn_bc.gif"});
	}
}
/* 通话质检, 打分 */
