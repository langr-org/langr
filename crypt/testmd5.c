	...
	if ( argc > 1 ) {
		for ( i = 1; i < argc; i++ ) {
			if ( argv[i][0] == '-' && argv[i][1] == 's') {
				tmp = md5(argv[i] + 2);
				printf("md5(%s)=", argv[i] + 2);
				md5_printf(tmp);
				printf("\n");

				md5_snprintf(buf, tmp);
				printf("md5_snprintf(buf, md5(%s))=%s\n", argv[i] + 2, buf);

				free(tmp);

				md5_string(result, argv[i] + 2, strlen(argv[i]) - 2);
				printf("md5_string(%s)=", argv[i] + 2);
				md5_printf(result);
				printf("\n");
			} /*else if ( strcmp(argv[i], "-t") ) {
				;
			} */else {
				md5_file(result, argv[i]);
				printf("md5_file(%s)=", argv[i]);
				md5_printf(result);
				printf("\n");
			}
		}
	} else {
		printf("usage: \tmd5  [filename] [-sString]\n");
	}
